...
{
    public partial class StaffAddressAddNew : System.Web.UI.Page
    {
        #region Variables
        ...
        protected void btnSave_OnClick(object sender, EventArgs e)
        {
            ...
            {
                Save();
                Response.Redirect(UrlBuilderHelper.GetUrl(new ResourceData() { ResourceId=this.staffId}, NavigateAction.List, ResourceDataContentType.StaffAddress), true);
        ...
        private void Save()
        {
            ...
            if (this.item == null)
            {
                ...
                this.item.ContentType = (int)ResourceDataContentType.StaffAddress;
            ...
            this.item.Content = txtAddress.Text;// Request.Params[txtCode.UniqueID];
            this.item.SubContentType = int.Parse(drpType.SelectedValue);

            XmlHash xmlHash = new XmlHash();
            xmlHash.Set(AddressField.Address.ToString(), txtAddress.Text);
            xmlHash.Set(AddressField.Country.ToString(), drpCountry.SelectedValue);
            xmlHash.Set(AddressField.Province.ToString(), drpProvince.SelectedValue);
            xmlHash.Set(AddressField.Type.ToString(), drpType.SelectedValue);
            xmlHash.RootTag = ResourceDataContentType.StaffAddress.ToString();
        ...
        private void BindGuid()
        {
            ...
            drpCountry.SelectedValue = xmlhash.GetKeyFromHash(AddressField.Country.ToString());
            try
            {
                drpProvince.ParentId = int.Parse(drpCountry.SelectedValue);
                drpProvince.ReBindData();
            ...
            drpProvince.SelectedValue = xmlhash.GetKeyFromHash(AddressField.Province.ToString());
            drpType.SelectedValue = xmlhash.GetKeyFromHash(AddressField.Type.ToString());

            txtAddress.Text = this.item.Content;
