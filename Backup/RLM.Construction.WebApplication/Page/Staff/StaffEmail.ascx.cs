...
{
    public partial class StaffEmail : System.Web.UI.UserControl
    {
        #region Properties
        ...
        protected void rptItems_OnItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            try
            {
                ...
                ltrInfoType.Text = Utility.GetEnumValue<EmailType>((EmailType)NumberHelper.GetValue<int>(dataItem.SubContentType));
                ...
                try
                {
                    ...
                    bool isUsedEmail = bool.Parse(xmlHash.GetKeyFromHash(EmailField.IsCurrentUsed.ToString()));
                    if (isUsedEmail == true)
                    {
        ...
        private void LoadData()
        {
            ...
            lnkAddNewItem.HRef = UrlBuilderHelper.GetUrl(new ResourceData() { ResourceId = this.ItemId }, NavigateAction.AddNew, ResourceDataContentType.StaffEmail);
        }

        private void BindData()
        {
            TList<ResourceData> items = ServiceRepositoryHelper.ResourceDataServiceHelper.GetByResource(this.ItemId,ResourceType.Staff,ResourceDataContentType.StaffEmail);
