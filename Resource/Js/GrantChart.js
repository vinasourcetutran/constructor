...
function Gantt(gDiv) {
    ...
    this.Draw = function() {
        ...
        if (_taskList.length > 0) {
            ...
            _gStr = "";
            _gStr += "</tr><tr>";
            _thirdRow = "<tr><td class='GTaskName'>&nbsp;</td>";
            ...
            while (Date.parse(_dTemp) <= Date.parse(_maxDate)) {
                if (_dTemp.getDay() % 6 == 0) //Weekend
                {
                    ...
                    if (Date.parse(_dTemp) == Date.parse(_currentDate))
                        ...
                        _thirdRow += "<td id='GC_" + (counter++) + "' class='GWeekend' style='height:" + (_taskList.length * 24) + "'>&nbsp;</td>";
                }
                else {
            ...
            for (i = 0; i < _taskList.length; i++) {
                ...
                _gStr += "<div style='position:absolute; top:" + (20 * (i + 2)) + "; left:" + (_offSet * 27 + 304) + "; width:" + (27 * _dateDiff - 1 + 100) + "'><div title='" + _taskList[i].getTask() + "' class='GTask' style='float:left; width:" + (27 * _dateDiff - 1) + "px;'>" + getProgressDiv(_taskList[i].getProgress()) + "</div><div style='float:left; padding-left:3'>" + _taskList[i].getResource() + "</div></div>";
                var str = StringHelper.trim(_taskList[i].getTask(), 55);
                _gStr += "<div style='position:absolute; top:" + (20 * (i + 2) + 10) + "; left:5px;'><label title='" + _taskList[i].getTask() + "'>" + str + "</label></div>";
            }
            _GanttDiv.innerHTML = _gStr;
            _GanttDiv.className = 'GrantRowTitle';
        }
    }
}
